...
import pe.edu.upeu.microserviceenviroment.infrastructure.adapters.input.rest.mapper.StateRestMapper;
...
public class StateRestAdapter {
    ...
    public ResponseEntity<StateResponse> create(@Valid @RequestBody StateCreateRequest request){
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(restMapper.toStateResponse(
                        servicePort.save(restMapper.toState(request))));
    }
